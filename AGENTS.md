...
4. **Never print secret values.** Use agent-safe ops only.
5. **PRs target `develop`.** Never merge or push `main` (CI/CD-only release path).

## Identity

Secret management helpers implementing the dc/Infisical flow used across trl-infra (dc/secret-bucket layering: dc: → override: → auto: → default:). Never print secret values; use agent-safe ops. Full reference: trl-infra docs/secret-management.md.
